...
| 25 | 1,804 | 11% | enemy_extras (`enemy_type_short_name_copy`, slime split / SLIME_BIG death pop, gorgon snake summon). **Evicted from bank 0 on 2026-07-27** — autobank had been parking this no-`#pragma bank` module in HOME, and widening `enemy_hp[]` to `uint16_t` grew its three spawn-time stores enough to overflow bank 0 into bank 1. Now pinned with `#pragma bank 25`; every function is BANKED in `enemy.h`, so callers far-call it either way. Recovered ~1.77 KB of HOME. |
...
Fixed WRAM (0xC000–0xDFFF): `_DATA` = 7,558 B + `_INITIALIZED` 45 B, ending 0xDE53 (re-measured
2026-07-27, after `enemy_hp[]` widened to `uint16_t` for the HP-cap lift: +23 B) → **429 B stack
headroom — this is load-bearing, treat it as a floor**:
