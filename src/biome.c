...
BANKREF_EXTERN(biome_encounter_copy_defs)
BANKREF_EXTERN(biome_encounter_load_palettes)
...
static const BiomeEntry biome_table[BIOME_COUNT] = {
    ...
    /* BIOME_ENCOUNTER */ { BANK(biome_encounter_copy_defs), biome_encounter_copy_defs, biome_encounter_load_palettes },
};
...
uint8_t biome_pick_for_floor(uint8_t floor_n, uint16_t seed) {
    ...
    if (floor_n == ENCOUNTER_FLOOR) return BIOME_ENCOUNTER; // the one shared '?' encounter slot (49)
    if (floor_n >= TOWN_FLOOR_BASE) return BIOME_TOWN; // town interiors (46-48)
